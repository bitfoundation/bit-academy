...
Selector: به منظور اضافه نمودن استایل به یک المنت، باید آن المنت را انتخاب کنیم. یکی از روش های انتخاب المنت در CSS، استفاده از نام خود المنت می‌باشد.  برای مثال در تصویر بالا ما از نام تگ button به عنوان سلکتور استفاده کرده‌ایم،  به  این معنا که می‌خواهیم استایلی را برای تمامی  button  های موجود در صفحه تعیین کنیم. 

 Property: با استفاده از Property ها، ویژگی‌های نمایش یک المنت در HTML  را تعیین می‌کنیم. به عنوان مثال اگر بنویسیم background-color: red به این معناست که رنگ پس زمینه تمامی button های موجود در صفحه قرمز باشد.

Property value: برای هر Property یا ویژگی در CSS باید یک مقدار در نظر گرفته شود. به عنوان مثال در این تصویر، رنگ قرمز را برای پراپرتی background-color المنت یا تگ button در نظر گرفتیم.
...
ویژگی id،  یک شناسه را برای یک المنت مشخص می کند. مقداری که به ویژگی  id اختصاص می‌دهیم باید در فایل HTML منحصر به فرد باشد.

از ویژگی id بیشتر برای تغییر دادن المنت ‌ها و یا محتوای داخل المنت ‌ها توسط JavaScript در زمان اجرا استفاده می‌کنیم.
...
در اینجا let یک کلمه کلیدی است که برای تعریف کردن متغییر استفاده می‌شود. متغیر می‌تواند حاوی هر مقداری باشد، که در اینجا ما قصدمان داشتن متغیری است که به المنت #count اشاره می‌کند، که در ادامه بتوانیم با کلیک بر روی دکمه‌های Increase و Decrease، مقدار داخل آنرا کم و زیاد کنیم.
